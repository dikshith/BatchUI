...
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_NAME'] = 'profile_session'
...
def ensure_unique_session():
    ...
    # Clean up old sessions periodically (but less frequently)
    if random.random() < 0.1:  # Only run cleanup 10% of the time
...
def compare_profiles_direct():
    """Compare profiles directly from uploaded files"""
    ...
    for i, file in enumerate(files):
        ...
        if not allowed_file(file.filename):
            return jsonify({'error': f'Invalid file type for {file.filename}. Please upload .prof, .pstats, or .pkl files'}), 400
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=f'_{file.filename}') as temp_file:
            ...
            # Parse the profile
            profile_data = parse_profile_file(temp_file.name)
    ...
    # Build comparison data using the same logic as the original compare endpoint
    comparison_data = {
        'profiles': [],
        'comparison': {
    ...
    if len(parsed_profiles) >= 2:
        times = [p['data'].get('total_time', 0) for p in parsed_profiles]
        calls = [p['data'].get('total_calls', 0) for p in parsed_profiles]
        ...
        # Function analysis
        all_functions = {}
        for i, profile in enumerate(parsed_profiles):
        ...
        common_funcs = []
        unique_funcs = {f'profile_{i+1}': [] for i in range(len(parsed_profiles))}
        
        for func_name, func_data in all_functions.items():
            if len(func_data) == len(parsed_profiles):
                common_funcs.append({
                    'name': func_name,
                    'profiles': func_data
                })
...
def compare_profiles():
    ...
    if not profiles:
        return jsonify({'error': 'No profiles found for comparison'}), 400
...
@app.route('/api/remove_profile/<profile_slot>', methods=['POST', 'DELETE'])
def remove_profile(profile_slot):
    """Remove a specific profile by slot"""
    try:
        ...
        profiles = get_user_profiles_from_storage()
        profile_key = profile_slot  # Use the slot directly, not f'profile_{profile_slot}'
        
        # Debug logging
        print(f"Attempting to remove profile: {profile_key}")
        print(f"Available profiles: {list(profiles.keys())}")
        print(f"Session ID: {session.get('session_id')}")
        
        if profile_key in profiles:
            # Get the profile info before deleting
            ...
            filepath = profile_info.get('filepath')
            if filepath and os.path.exists(filepath):
                try:
                    os.remove(filepath)
                    print(f"Deleted profile file: {filepath}")
                except Exception as e:
                    print(f"Error deleting file {filepath}: {e}")
            
            return jsonify({'message': f'Profile {profile_slot} removed successfully'}), 200
        
        return jsonify({'message': f'Profile {profile_slot} not found'}), 404
