...
export class PythonProfileService {
  ...
  uploadProfile(
    ...
    return this.http.post<ProfileUploadResponse>(
      `${this.apiUrl}/upload`,
      formData,
      {
  ...
  compareProfilesDirect(files: File[]): Observable<ComparisonResponse> {
    const formData = new FormData();
    files.forEach(file => {
  ...
  removeProfile(profileSlot: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/remove_profile/${profileSlot}`, {
      headers: this.headers,
      withCredentials: true,
    });
  }
