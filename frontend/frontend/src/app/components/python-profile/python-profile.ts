...
export class PythonProfileComponent implements OnInit {
  ...
  public onFileSelected(event: Event, profileIndex: number): void {
    ...
    if (file) {
      // Validate file type
      const allowedExtensions = ['prof', 'pstats', 'pkl'];
      const fileExtension = file.name.split('.').pop()?.toLowerCase();
      
      if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
        alert('Invalid file type. Please upload .prof, .pstats, or .pkl files');
        return;
      }
  ...
  public removeProfile(profileIndex: number): void {
    ...
    this.profileSlots[profileIndex].name = `Profile ${profileIndex + 1}`;
    console.log(`Profile ${profileIndex + 1} removed`);
  }
  ...
  public runComparison(): void {
    const uploadedFiles = this.profileSlots
      .filter(slot => slot.file !== null)
      .map(slot => slot.file!)
      .filter(file => file.size > 0); // Filter out empty files

    if (uploadedFiles.length < 2) {
      console.error('Need at least 2 files to compare');
      return;
    }
    ...
    this.profileService
      .compareProfilesDirect(uploadedFiles)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.comparisonResults.set({
            summary: {
              totalFunctions: response.profiles.reduce(
                (sum: number, p: any) => sum + p.function_count,
                0
              ),
              ...
              performanceGain: this.calculatePerformanceGain(
                response.comparison.performance_metrics
              ),
            },
  ...
  private categorizeFunction(funcName: string): {
    ...
    // Check for built-in
    const isBuiltin =
      builtinPatterns.some((pattern) => name.includes(pattern.toLowerCase())) ||
      (funcName.includes('__') && funcName.endsWith('__'));

    // Check for standard library
    const isStdlib = stdlibPatterns.some((pattern) =>
  ...
  public getFilteredFunctions(): FunctionProfile[] {
    ...
    fns = fns.filter((f) =>
      Object.values(f.profiles).some(
        (p) =>
          p != null &&
          (p as ProfileData).cumulative_time >= this.filterThreshold
      )
    );
    ...
    fns.sort((a, b) => {
      ...
      switch (this.sortBy) {
        case 'function':
          ...
          break;
        case 'cumTime':
          aValue = this.getMaxProfileValue(a, 'cumulative_time');
          bValue = this.getMaxProfileValue(b, 'cumulative_time');
          break;
        case 'calls':
          aValue = this.getMaxProfileValue(a, 'calls');
          bValue = this.getMaxProfileValue(b, 'calls');
          break;
        case 'perCall':
          aValue = this.getMaxProfileValue(a, 'per_call');
          bValue = this.getMaxProfileValue(b, 'per_call');
          break;
        default:
          return 0;
      }
  ...
  sortResults() {}
  applyFilter() {}
  selectFunction(func: FunctionProfile) {
  ...
  formatTime(seconds: number): string {
    ...
    if (seconds >= 0.001)
      return `${(seconds * 1000).toFixed(this.displayPrecision)}ms`;
    ...
    metrics.forEach((func) => {
      ...
      if (profile1 && profile2) {
        ...
        const timeDiff = Math.abs(
          profile1.cumulative_time - profile2.cumulative_time
        );
        ...
        if (timeDiff > 0.1) {
          ...
          insights.push({
            ...
            impact: 'High',
            function: func.name,
            description: `Significant time difference: ${this.formatTime(
              profile1.cumulative_time
            )} vs ${this.formatTime(profile2.cumulative_time)}`,
          });
        }
        ...
        if (callsDiff > 1000) {
          insights.push({
            ...
            impact: 'Medium',
            function: func.name,
            description: `Call count difference: ${this.formatCalls(
              profile1.calls
            )} vs ${this.formatCalls(profile2.calls)}`,
          });
        }
      } else {
        // Function only exists in one profile
        const profile = profile1 || profile2;
        if (profile && profile.cumulative_time > 0.01) {
          // Only show if it takes more than 10ms
          insights.push({
            type: 'info',
            impact: 'Low',
            function: func.name,
            description: `Function only exists in ${
              profile1 ? 'Profile 1' : 'Profile 2'
            } with ${this.formatTime(profile.cumulative_time)} execution time`,
          });
        }
      }
    });
  ...
  exportResults() {
    ...
    a.href = url;
    a.download = 'profile-comparison-results.json';
    a.click();
